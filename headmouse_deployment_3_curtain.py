...
ser.dtr = False
ser.rts = False
time.sleep(2)  # ESP32 reset even laten gebeuren

curtain_status = True # True = open, False = closed
curtain_released = False
curtain_closed_time = 0
face_detected = True
last_face_time = int(time.time() * 1000)
face_first_seen_time = 0

INACTIVITY_THRESHOLD = 5000
HOLD_OPEN_THRESHOLD = 60000
...
with FaceLandmarker.create_from_options(options_landmarker) as landmarker:
    ...
    while video_capture.isOpened():
        ...
        if face_detected:
            ...
            time_present = current_time - face_first_seen_time
            ...
            if not curtain_status and not curtain_released and time_present >= HOLD_OPEN_THRESHOLD:
                ser.write(b'o')
                print("gordijn open - vrijgegeven na 20s")
                curtain_status = True
        ...
        #if (current_time - last_face_time > INACTIVITY_THRESHOLD) and (curtain_status == False):
        #    ser.write(b'o')
        #    print("open")
        #    curtain_status = True
